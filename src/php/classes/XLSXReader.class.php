...
class XLSXReader {
  ...
  public function getSheetNameByNumber($sheetNo) {
    foreach($this->sheetInfo as $sheetName=>$sheetInfo) {
      if($sheetInfo['rid'] === "rId$sheetNo" or strpos(pathinfo($sheetInfo['path'], PATHINFO_FILENAME), "sheet$sheetNo") !== false) {
        return $sheetName;
      }
    }
    throw new Exception("Sheet number $sheetNo does not exist in the Excel file");
  }
